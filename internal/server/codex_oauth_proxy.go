...
func (s *Server) doForwardCodexOAuth(c *gin.Context, a *auth.Auth, path string, body []byte, stream bool, model, clientToken, clientName, slotID string, start time.Time, attempts int) (retry, done bool) {
	...
	// custom header at all. One curl user with a chatty SDK was enough to make
	// this deployment's requests a shape nothing else in the world produces.
	//
	...
	default:
		...
		// more; the keepalive commits a 200 and heartbeats once it starts.
		...
		if isChat {
			...
			if cerr != nil {
				...
				if ka.Committed() {
					ka.Fail("service_response_error", "The model service returned an incomplete response. Please try again.")
				} else {
				...
				if len(payload) > 0 && payload[0] == '{' {
					...
					if err := json.Unmarshal(payload, &ev); err == nil {
						...
						case "response.completed", "response.incomplete":
							...
							mergeCodexUsage(counts, extractCodexBackendUsageFromJSON(payload))
							payload, perr := output.Patch(ev.Response)
							return payload, "", perr
						}
					}
				}
